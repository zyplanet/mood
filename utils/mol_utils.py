...
def load_smiles():
    ...
    with open(f'data/valid_idx_zinc250k.json') as f:
...
def get_novelty_in_df(df):
    ...
    if 'sim' not in df.keys():
        ...
        train_fps = torch.load(f"data/train_fps.pt")
        max_sims = []
...
def get_sim(smiles1,smiles2,s2train=False):
    mols1 = [Chem.MolFromSmiles(smi) for smi in smiles1]
    mols2 = [Chem.MolFromSmiles(smi) for smi in smiles2]
    fps1 = [AllChem.GetMorganFingerprintAsBitVect((mol), 2, 1024) for mol in mols1]
    fps2 = [AllChem.GetMorganFingerprintAsBitVect((mol), 2, 1024) for mol in mols2]
    train_fps = torch.load(f"data/train_fps.pt")
    s1s2sim = []
    s1trainsim = []
    s2trainsim = []
    for fp in tqdm(fps1):
        ...
        s1s2sim.append(max(sims))
        sims = DataStructs.BulkTanimotoSimilarity(fp, train_fps)
        s1trainsim.append(max(sims))
    if s2train:
        for fp in tqdm(fps2):
            sims = DataStructs.BulkTanimotoSimilarity(fp, train_fps)
            s2trainsim.append(max(sims))
    else:
        s2trainsim=None
    return s1s2sim,s1trainsim,s2trainsim
from tqdm import tqdm
def write_train_fps():
    train_smiles,_ = load_smiles()
    print("to mol")
    train_mols = [Chem.MolFromSmiles(smi) for smi in tqdm(train_smiles)]
    print("to fps")
    train_fps = [AllChem.GetMorganFingerprintAsBitVect((mol), 2, 1024) for mol in tqdm(train_mols)]
    save_path = f'data/train_fps.pt'
    torch.save(train_fps,save_path)
